...
# -v or --version
echo "Using `basename $0` version $ZS_VERSION (build: \$Revision: 121898 $)" >> $LOG_FILE
if [ "$1" = "-v" -o "$1" = "--version" ]; then
	echo "`basename $0` version $ZS_VERSION (build: \$Revision: 121898 $)"
	usage
	exit 0
fi
...
# Ubuntu 18.04 checks
if echo $CURRENT_OS | egrep -q "Ubuntu 18" ; then
	if ! apt-cache policy execstack | grep -q universe; then
		if ! grep ^"deb " /etc/apt/sources.list | grep -q universe; then
			echo "Ubuntu universe repository is required for installing Zend Server";
			echo "Please enable it first and try to install again";
			exit 2
		fi
	fi
fi
